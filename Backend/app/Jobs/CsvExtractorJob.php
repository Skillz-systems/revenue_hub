...
class CsvExtractorJob implements ShouldQueue
{
    ...
    private function property($data)
    {
        ...
        $getOccupant = $data[2];
        $getAddress = $data[3];
        $getStreet = (new StreetService())->getStreetFromStreetName($data[4]);
        $getAssetNumber = $data[5];
        $getCadastralZone = (new CadastralZoneService())->getCadastralZoneFromZoneName($data[6]);
        $getPropertyType = (new PropertyTypeService())->getPropertyTypeFromPropertyTypeName($data[7]);
        $getPropertyUse = (new PropertyUseService())->getPropertyUseFromPropertyUseName($data[8]);
        $getRatingDistrict = (new RatingDistrictService())->getRatingDistrictFromDistrictName($data[9]);
        $getAnnualValue = str_replace(",", "", $data[10]);
        $getRatePayable = str_replace(",", "", $data[11]);
        $getArrears = str_replace(",", "", $data[12]);
        $getPenalty = str_replace(",", "", $data[13]);
        $getGrandTotal = str_replace(",", "", $data[14]);
        $getCategory = (new CategoryService())->getCategoryFromCategoryName($data[15]);
        $getGroup = (new GroupService())->getGroupFromGroupName($data[16]);
        $getActive = $data[17];
        $propertyData = [
            "pid" => $getPid,
