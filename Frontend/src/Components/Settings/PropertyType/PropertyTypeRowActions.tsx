...
interface PropertyTypeRowActionsProps {
  data: {
    id: string;
    name: string;
  };
...
const PropertyTypeRowActions: React.FC<PropertyTypeRowActionsProps> = ({ data, onActionComplete }) => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await apiCall({
        endpoint: `property-type/update/${data.id}`,
        method: 'put',
        data: { name: form.name },
      });
  ...
  const handleDelete = async () => {
    try {
      const response = await apiCall({
        endpoint: `property-type/delete/${data.id}`,
        method: 'delete',
      });
  ...
  const handleView = async () => {
    try {
      const response = await apiCall({
        endpoint: `property-type/view/${data.id}`,
        method: 'get',
      });
      if (response.status === 200) {
      ...
      {updateData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 ">
          <div className="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <div>
              <div className="mb-4">
                <label className="block font-semibold mb-2">Property Type Name</label>
                <input
                  type="text"
                  name="name"
                  value={form.name}
...
export default PropertyTypeRowActions;
