...
interface PropertyUseRowActionsProps {
  data: {
    id: string;
    name: string;
  };
...
const PropertyUseRowActions: React.FC<PropertyUseRowActionsProps> = ({ data, onActionComplete }) => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await apiCall({
        endpoint: `property-use/update/${data.id}`,
        method: 'put',
        data: { name: form.name },
      });
  ...
  const handleDelete = async () => {
    try {
      const response = await apiCall({
        endpoint: `property-use/delete/${data.id}`,
        method: 'delete',
      });
  ...
  const handleView = async () => {
    try {
      const response = await apiCall({
        endpoint: `property-use/view/${data.id}`,
        method: 'get',
      });
      if (response.status === 200) {
      ...
      {updateData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 ">
          <div className="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <div>
              <div className="mb-4">
                <label className="block font-semibold mb-2">Property Use Name</label>
                <input
                  type="text"
                  name="name"
                  value={form.name}
            ...
            <button onClick={handleUpdate} className="mt-4 px-4 py-2 bg-blue-800 text-white rounded">
              <GrUpdate />
            </button>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <div className="mt-4 flex justify-end">
              <button onClick={handleDelete} className="px-4 py-2 bg-blue-800 text-white rounded">
              <FaTrash />
              </button>
            </div>
          </div>
...
export default PropertyUseRowActions;
