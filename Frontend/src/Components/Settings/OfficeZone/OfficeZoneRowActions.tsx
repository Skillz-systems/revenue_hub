...
const OfficeZoneRowActions: React.FC<OfficeZoneRowActionsProps> = ({ data, onActionComplete }) => {
  ...
  const handleUpdate = async () => {
    try {
      const response = await apiCall({
        endpoint: `office-zone/update/${data.id}`,
        method: 'put',
        data: { name: form.name },
      });
  ...
  const handleDelete = async () => {
    try {
      const response = await apiCall({
        endpoint: `office-zone/delete/${data.id}`,
        method: 'delete',
      });
  ...
  const handleView = async () => {
    try {
      const response = await apiCall({
        endpoint: `office-zone/view/${data.id}`,
        method: 'get',
      });
      if (response.status === 200) {
      ...
      {updateData && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 ">
          <div className="bg-white rounded-lg shadow-lg p-6 w-1/3">
            ...
            <div>
              <div className="mb-4">
                <label className="block font-semibold mb-2">Office Zone Name</label>
                <input
                  type="text"
                  name="name"
                  value={form.name}
...
export default OfficeZoneRowActions;
