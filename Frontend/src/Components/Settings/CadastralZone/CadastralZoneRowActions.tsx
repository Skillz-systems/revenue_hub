...
interface CadastralZoneRowActionsProps {
  data: CadastralZone;
  onActionComplete: () => void;
  ratingDistricts: RatingDistrict[];
}

const CadastralZoneRowActions: React.FC<CadastralZoneRowActionsProps> = ({ data, onActionComplete, ratingDistricts }) => {
  ...
  const handleUpdate = async () => {
    try {
      ...
      const response = await apiCall({
        endpoint: `cadastral-zone/update/${data.id}`,
        method: 'put',
  ...
  const handleDelete = async () => {
    try {
      const response = await apiCall({
        endpoint: `cadastral-zone/delete/${data.id}`,
        method: 'delete',
      });

      if (response.status === 200 || response.status === 204) {
        console.log("Deleted cadastral zone with ID:", data.id);
        console.log("Deleted cadastral zone DATA:", data);
        onActionComplete();
        setDeleteData(false);
      } else {
  ...
  const renderUpdateForm = () => (
    <>
      <div className="mb-4">
        <label className="block font-semibold mb-2">Cadastral Zone Name</label>
        {renderFormField('name', 'text', 'Cadastral Zone Name')}
      </div>
      <div className="mb-4">
        <label className="block font-semibold mb-2">Rating District</label>
        {renderFormField('rating_district_id', 'dropdown', 'Rating District')}
      </div>
    </>
  );
              ...
              <table className='w-full'>
                <tbody>
                  ...
                  <tr className='border-b hover:bg-gray-50'>
                    ...
                    <td className='p-2'>{viewData.rating_district?.name}</td>
                  </tr>
                  <tr className='border-b hover:bg-gray-50'>
...
export default CadastralZoneRowActions;
